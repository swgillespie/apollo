...
extern crate apollo;
use apollo::{Engine, Color, Square, Move};
...
fn fools_mate_check() {
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("rnb1kbnr/pppp1ppp/8/4p3/6Pq/5P2/PPPPP2P/RNBQKBNR w KQkq - 0 1")
        .unwrap();
...
fn sliding_piece_pin() {
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("8/8/4q3/8/8/8/4P3/4K3 w - - 0 1").unwrap();
...
fn position_5_bug_1_absolute_pin() {
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("rnR2k1r/pp1qbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 0 1")
        .unwrap();
...
fn stalemate_smoke() {
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("7k/5K2/6Q1/8/8/8/8/8 b - - 0 1").unwrap();
...
fn fifty_move_rule_smoke() {
    let engine = Engine::new();
    let mut pos = engine.new_position_from_fen("8/7k/6R1/5K2/1r3B2/8/8/8 w - - 49 121").unwrap();
