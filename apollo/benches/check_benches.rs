...
use apollo::{Engine, Position, Color};
use test::Bencher;

#[bench]
fn starting_position_check_bench(b: &mut Bencher) {
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").unwrap();
...
fn fools_mate_checkmate_bench(b: &mut Bencher) {
    ...
    // expensive.
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("rnb1kbnr/pppp1ppp/8/4p3/6Pq/5P2/PPPPP2P/RNBQKBNR w KQkq - 0 1").unwrap();
...
fn midgame_negative_checkmate_bench(b: &mut Bencher) {
    ...
    let engine = Engine::new();
    let pos = engine.new_position_from_fen("r3k1nr/1b1p2pp/p2b2Q1/n3p1B1/2B5/2N2N2/PP3PPP/R3R1K1 b kq - 0 1").unwrap();
    b.iter(|| pos.is_checkmate());
}
